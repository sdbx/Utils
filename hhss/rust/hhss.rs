...
fn main() {
    ...
    if sentences.len() < MIN_SENTENCE_NUM {
        eprintln!(
            "hhss: it is required for {} to have valid lines more than or equal to {}.",
    ...
    if num_sentences > sentences.len() {
        eprintln!(
            "hhss: argv[1] = {} needs to be <= {}, the number of valid lines in {}.",
            num_sentences,
            sentences.len(),
            SENTENCE_FILE
        );
...
fn replace_templates(sentence: &str, users: &[String]) -> String {
    ...
    for (i, part) in parts.iter().enumerate() {
        ...
        if i < parts.len() - 1 {
            ...
            let selected_user = available_users.choose(&mut rand::rng()).unwrap();
